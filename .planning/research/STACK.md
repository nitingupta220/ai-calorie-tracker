# Technology Stack — AI Personal Coach for Indian Youth

**Project:** ai-calorie-weight-loss (codename)
**Researched:** 2026-05-27
**Overall confidence:** HIGH (Context7-verified for core libs, official-docs-verified for AI provider limits, web-verified for India-specific PaaS/storage)
**Locked context:** Bootstrap solo founder, Android-first (iOS V2), DPDP-compliant, free-first AI provider strategy is non-negotiable

---

## Executive Summary

This stack optimizes for **three hard constraints** the founder named:

1. **Free-first AI** — Phase 0/1 must run at near-₹0; paid only after free-tier RPD exhausted.
2. **India DPDP residency** — photo + PII storage in India jurisdiction (Cloudflare R2 India bucket, not Mumbai-region-S3).
3. **First-time RN shipper** — Expo + EAS Build minimizes native-config complexity; managed workflow keeps shipping risk inside founder's capability band.

**Three corrections to the design doc that the research surfaced:**
...
├── app/
│   ├── main.py              # FastAPI app
│   ├── api/                 # routers per domain (auth, meals, advice, users)
│   ├── core/                # config, db, security
│   ├── models/              # SQLAlchemy 2.0 models
│   ├── schemas/             # Pydantic v2 schemas
│   ├── ai_provider.py       # **AI abstraction — locked Week 0d**
│   ├── services/            # vision, advice, nutrition, dish_decomp
│   └── workers/             # background tasks (push scheduler, cleanup)
├── alembic/
├── tests/
└── pyproject.toml           # use uv or poetry
```

**Anti-pattern:** Do NOT use sync FastAPI handlers with sync DB driver — design doc's 100-user Phase 2 scale will choke on a single slow query. Async-first throughout.
...
**Anti-pattern:** S3 Mumbai region. Egress fees will exceed AI cost at scale. R2 + Cloudflare CDN is the bootstrap-grade choice.
...
**Anti-pattern:** Email/password auth as primary. Indian youth use phone numbers as primary identity. Email is V2 secondary at best.

**Anti-pattern:** Sign in with Google as primary on Android. Acceptable as secondary; phone-first matches design doc persona.

### AI Provider Stack (the non-negotiable, free-first layer)

The design doc names `ai_provider.py` as a Week 0d locked deliverable. Research-verified provider specs:
...
**Anti-pattern:** USDA FoodData Central as primary. American serving sizes + ingredient variations don't match Indian dishes. IFCT is non-negotiable for India persona.

**Anti-pattern:** Letting the LLM hallucinate macros directly from photo. Pipeline must be: photo → dish ID + portion estimate (LLM) → IFCT lookup (deterministic) → final macros. LLM-direct macros breach the ±35% gate.
...
**Anti-pattern:** Client-side scheduled local notifications as the only retention mechanism. Phone-killed/swiped notifications don't re-arm reliably on Android. Server-driven FCM is the source of truth for the retention loop.
...
**Razorpay fee model (verified):** 2% transaction fee + 18% GST on fee = effective 2.36%. On ₹299: ~₹7 fee, ~₹292 net. Founder must register GSTIN before annual receipts exceed ₹20L.

**Anti-pattern:** Stripe India. Lower acceptance rate on Indian cards/UPI; Razorpay's local-bank integration depth wins for this persona.

**Anti-pattern:** Card e-mandates over UPI Autopay for ₹299 tier. UPI Autopay has lower failure rate and lower transaction cost at sub-₹500 tickets.
...
npx expo install expo-camera expo-image-picker expo-image-manipulator \
                 expo-notifications expo-router expo-secure-store \
                 expo-network expo-updates expo-dev-client
npm install @tanstack/react-query@^5 zustand@^5 \
            ...
            @sentry/react-native
npx sentry-wizard -i reactNative -p android  # automated Sentry setup
```
...
r2 = boto3.client(
    's3',
    endpoint_url=f'https://{account_id}.in.r2.cloudflarestorage.com',
...
This stack supports the design doc's 14-week plan unchanged, with three corrections to bake into Week 0d:
...
No phase needs deeper stack research; phase-specific research (e.g., prompt engineering, advice rubric calibration) is the next research surface — not stack.
